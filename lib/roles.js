...
const getAllRoles = () => {
    ...
    return new Promise((res, rej) => {
        db.query(sql, (err, rows) => {
            ...
            if (rows.length === 0) {
                res({Result:'No Roles in Database'})
            } else {
                res(rows)
            }
        });
...
const addInitialRole = async () => {
    await inquirer.prompt([
        {
            type: 'text',
            name: 'title',
            message: 'Please enter role of first employee that you will add.',
            validate: roleTitleInput => {
                if (roleTitleInput){
                    return true;
                } else {
        ...
        },
        {
            type: 'list',
            name: 'depId',
            message: 'Please select the department associated with this role for the first employee to be added.',
            choices: await getAllDepartments().then(res => 
                res.map(departments => {
                return `${departments.id}. ${departments.name}`;
                    }
...
module.exports = {getAllRoles, addRole, addInitialRole}