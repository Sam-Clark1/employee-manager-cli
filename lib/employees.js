...
const addInitialEmployee = async () => {
    await inquirer.prompt([
        {
            type:'text',
            name: 'first_name',
            message: "Please enter the first employee's first name.",
            validate: firstNameInput => {
                if (firstNameInput){
                    return true;
                } else {
    ...
    .then(data => {
        const { first_name, last_name, roleId } = data;
        const role_id = roleId.split('.')[0];

        const sql = 'INSERT INTO employees (first_name, last_name, role_id) VALUES (?, ?, ?);'

        return new Promise((res, rej) => {
            db.query(sql, [first_name, last_name, role_id], (err, rows) => {
                if (err) {
                    console.log('Error occured')
                    rej(err)
                }
...
module.exports = { getAllEmployees, addEmployee, editEmployee, addInitialEmployee }